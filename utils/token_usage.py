...
class TokenUsage:
    ...
    def log_tokens(self, agent_name: str, input_tokens: int, output_tokens: int):
        ...
        if agent_total > AGENT_DEFAULT_TOKEN_LIMIT:
            raise Exception(f"Token limit exceeded for agent '{agent_name}' ({agent_total}/{AGENT_DEFAULT_TOKEN_LIMIT})")
        ...
        if agent_total >= 0.8 * AGENT_DEFAULT_TOKEN_LIMIT:
            logger.warning(f"[{agent_name}] Nearing agent token limit ({agent_total}/{AGENT_DEFAULT_TOKEN_LIMIT})")
